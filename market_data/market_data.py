...
class MarketData:
    ...
    def get_batch_data(self, symbols: List[str], start_date: datetime, end_date: datetime, force_refresh: bool = False) -> Dict[str, HistoricalData]:
        ...
        for symbol in symbols_to_fetch:
            try:
                print(f"\n🔍 YAHOO FINANCE API CALL: Fetching {symbol} from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} (FULL FETCH)")
    ...
    def get_batch_data_with_incremental_update(self, symbols: List[str], start_date: datetime, end_date: datetime, 
                                             force_refresh: bool = False, is_daily_update: bool = False) -> Dict[str, HistoricalData]:
        """
        Enhanced version of get_batch_data that handles incremental updates for daily data fetching.
        Only fetches missing data and merges with existing cache.
        """
        results = {}
        ...
        for symbol in symbols:
            try:
                ...
                existing_data = self._load_cached_data(symbol)
                existing_dates = set()
                
                if existing_data and existing_data.data_points:
                    existing_dates = {dp.date for dp in existing_data.data_points}
                    print(f"📦 CACHE: {symbol} has {len(existing_dates)} existing data points")
                
                # Calculate which dates are actually missing
                missing_dates = self._calculate_missing_dates(start_date, end_date, existing_dates)
                
                if not missing_dates and not force_refresh:
                    print(f"✅ CACHE HIT: {symbol} - no missing dates in requested range")
                    if existing_data:
                        results[symbol] = existing_data
                        self.data_cache[symbol] = existing_data
                    continue
                
                # Determine the actual fetch range (only missing dates)
                if missing_dates and not force_refresh:
                    ...
                    print(f"🎯 INCREMENTAL: {symbol} - fetching only {len(missing_dates)} missing dates from {fetch_start} to {fetch_end}")
                else:
                    fetch_start_dt = start_date
                    fetch_end_dt = end_date
                    print(f"🔄 FULL FETCH: {symbol} - fetching from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
                
                # Fetch data from API
                new_data_points = self._fetch_from_api(symbol, fetch_start_dt, fetch_end_dt, is_daily_update)
                
                if new_data_points:
                    # Merge new data with existing cache
                    merged_data = self._merge_data_points(existing_data, new_data_points, symbol)
                    results[symbol] = merged_data
                    self.data_cache[symbol] = merged_data
                    
                    # Save merged data to cache file
                    self._save_to_cache(symbol, merged_data)
                    
                    new_count = len(new_data_points)
                    total_count = len(merged_data.data_points)
                    print(f"✅ MERGED: {symbol} - added {new_count} new points, total {total_count} points")
                elif existing_data:
    ...
    def _load_cached_data(self, symbol: str) -> Optional[HistoricalData]:
        """Load cached data for a symbol from memory or file"""
    ...
    def _calculate_missing_dates(self, start_date: datetime, end_date: datetime, existing_dates: set) -> set:
        """Calculate which trading dates are missing from the cache"""
        missing = set()
        current = start_date
    ...
    def _fetch_from_api(self, symbol: str, start_date: datetime, end_date: datetime, is_daily_update: bool) -> List[DataPoint]:
        """Fetch data from Yahoo Finance API"""
        print(f"\n🔍 YAHOO FINANCE API CALL: {symbol} from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} ({'DAILY UPDATE' if is_daily_update else 'FETCH'})")
        ...
        if 'timestamp' not in result or not result['timestamp']:
            print(f"⚠️ No timestamps in response for {symbol}")
            return []
    ...
    def _merge_data_points(self, existing_data: Optional[HistoricalData], new_points: List[DataPoint], symbol: str) -> HistoricalData:
        """Merge new data points with existing data, deduplicating by date"""
        # Create a dict keyed by date for deduplication
        data_by_date = {}
        
        # Add existing data points
        if existing_data and existing_data.data_points:
            for dp in existing_data.data_points:
                data_by_date[dp.date] = dp
        
        # Add/update with new data points (new data takes precedence)
        for dp in new_points:
            data_by_date[dp.date] = dp
        
        # Sort by date and create new HistoricalData
        sorted_points = sorted(data_by_date.values(), key=lambda x: x.date)
        
        return HistoricalData(symbol=symbol, data_points=sorted_points)
    
    def _save_to_cache(self, symbol: str, data: HistoricalData) -> None:
    ...
    def _process_single_symbol_data(self, data: pd.DataFrame, symbol: str, results: Dict[str, HistoricalData]):
        ...
        if data_points:
            ...
            with open(cache_file, 'w') as f:
                json.dump(historical_data.to_dict(), f)
            print(f"Merged data: {symbol} with {len(data_points)} data points")
